class TaskApplication < ActiveRecord::Base
  ...
  def self.get_application(order_id)
    TaskApplication.joins('INNER JOIN requests ON task_applications.order_id=
        ...
        'task_applications.order_id=?', order_id).select('task_applications.order_id as order_id, user_profiles.name as name, user_profiles.occupation as occupation, user_profiles.id as carrier_id')
  end
